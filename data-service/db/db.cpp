...
{
    std::string insert_into_holdings = read_file("insert_into_holdings.sql");
    ...
    for (size_t i = 0; i != raw_fund_data["fund"].size(); ++i)
    {
        ...
        sqlite3_bind_text(pStmt, 4, raw_fund_data["date"][i].c_str(), raw_fund_data["date"][i].size() + 1, NULL);
